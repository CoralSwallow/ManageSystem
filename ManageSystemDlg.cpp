...
CManageSystemDlg::CManageSystemDlg(CWnd* pParent /*=nullptr*/)
...
BEGIN_MESSAGE_MAP(CManageSystemDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_Login, &CManageSystemDlg::OnBnClickedButtonLogin)
...
void CManageSystemDlg::OnBnClickedButtonLogin()
{
	// TODO: 在此添加控件通知处理程序代码
	CString usename, password;
	GetDlgItemText(IDC_EDIT_Username, usename);
	GetDlgItemText(IDC_EDIT_Password, password);
	if (usename.IsEmpty() || password.IsEmpty())
	{
		MessageBox(_T("用户名或密码不能为空"), _T("登陆失败"));
		return;
	}
	if (!usename.Compare(_T("admin")) && !password.Compare(_T("123456")))
	{
		ShowWindow(SW_HIDE);
		SystemDiag Diag;
		Diag.DoModal();
	...
	{
		MessageBox(_T("用户名或密码错误"), _T("登陆失败"));
	}
}
